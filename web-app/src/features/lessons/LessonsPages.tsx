...
export function LessonsPage() {
  ...
  const visible = useMemo(() => (lessons.data ?? []).filter((lesson) => {
    ...
    const haystack = `${lesson.childName} ${lesson.specialistName}`.toLocaleLowerCase();
...
export function LessonsTable({ lessons }: { lessons: Lesson[] }) { return <div className="table-wrap"><table><thead><tr><th>Дата и время</th><th>Ребёнок</th><th>Специалист</th><th>Статус</th><th>Длительность</th><th>Упражнения</th><th /></tr></thead><tbody>{lessons.map((lesson) => <tr key={lesson.id}><td>{formatDate(lesson.startedAt)}</td><td>{lesson.childName}</td><td>{lesson.specialistName}</td><td>{lessonStatus(lesson.status)}</td><td>{formatDuration(lesson.durationMs)}</td><td>{lesson.exerciseCount}</td><td><Link className="table-link" to={`/lessons/${lesson.id}`}>Открыть</Link></td></tr>)}</tbody></table></div>; }
...
export function LessonDetailsPage() {
  ...
  return <><div className="page-heading"><div><Link className="back-link" to="/lessons">← Занятия</Link><p className="eyebrow">Карточка занятия</p><h1>{lesson.childName}</h1><p className="muted">Со специалистом {lesson.specialistName}</p></div>{lessonStatus(lesson.status)}</div>
    <div className="detail-grid"><section className="panel"><h2>Сводка</h2><dl className="details"><dt>Начато</dt><dd>{formatDate(lesson.startedAt)}</dd><dt>Завершено</dt><dd>{formatDate(lesson.completedAt)}</dd><dt>Длительность</dt><dd>{formatDuration(lesson.durationMs)}</dd><dt>Специалист</dt><dd>{lesson.specialistName}</dd><dt>Ребёнок</dt><dd>{lesson.childName}</dd></dl><hr /><h2>Планшеты</h2><dl className="details"><dt>Специалист</dt><dd>{details.data.specialistDeviceName}</dd><dt>Ребёнок</dt><dd>{details.data.childDeviceName}</dd></dl></section><section className="panel"><h2>Упражнения</h2>{exercises.length ? <div className="table-wrap lesson-exercise-table"><table><thead><tr><th>#</th><th>Упражнение</th><th>Статус</th><th>Попытки</th><th>Ошибки</th><th>До правильного</th></tr></thead><tbody>{exercises.map((exercise) => <tr key={exercise.position}><td>{exercise.position}</td><td>{exercise.instructionText}</td><td>{exercise.status}</td><td>{exercise.attemptCount}</td><td>{exercise.incorrectAttempts}</td><td>{formatDuration(exercise.timeToCorrectMs)}</td></tr>)}</tbody></table></div> : <div className="empty-inline">Данные упражнений появятся после первого показа занятия.</div>}</section></div>
  </>;
}
