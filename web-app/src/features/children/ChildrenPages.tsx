...
export function ChildDetailsPage() {
  ...
  const item = child.data; const editable = canManage(role); const crmManaged = item.source?.crmManaged === true;
  return <><div className="page-heading"><div><Link className="back-link" to="/children">← Дети</Link><p className="eyebrow">Карточка ребёнка</p><h1>{fullName(item)}</h1><p className="muted">{ageOrBirthDate(item.birthDate)}</p></div><CatalogBadge status={item.status} /></div><div className="detail-grid"><section className="panel"><h2>Основные данные</h2><dl className="details"><dt>Имя</dt><dd>{item.firstName}</dd><dt>Фамилия</dt><dd>{item.lastName ?? "—"}</dd><dt>Дата рождения</dt><dd>{item.birthDate ? new Intl.DateTimeFormat("ru-RU", { dateStyle: "long" }).format(new Date(`${item.birthDate}T00:00:00`)) : "—"}</dd><dt>Добавлен</dt><dd>{formatDate(item.createdAt)}</dd><dt>Статус</dt><dd><CatalogBadge status={item.status} /></dd></dl>{item.source && <><hr /><h2>Источник данных</h2><dl className="details"><dt>Источник</dt><dd>{item.source.provider}{item.source.integrationStatus !== "ACTIVE" ? " · CRM отключена" : ""}</dd><dt>Последняя синхронизация</dt><dd>{formatDate(item.source.lastSyncedAt)}</dd></dl>{crmManaged && <p className="inline-note">Имя, фамилия, дата рождения и статус управляются CRM.</p>}</>}<hr /><h2>История занятий</h2>{lessons.isLoading ? <p className="muted">Загружаем…</p> : lessons.data?.length ? <LessonsTable lessons={lessons.data} /> : <div className="empty-inline">Занятий пока нет</div>}</section><section className="panel"><h2>Редактирование</h2>{editable && !crmManaged ? <form className="form-stack" onSubmit={(event) => { event.preventDefault(); update.mutate(); }}><label className="field"><span>Имя</span><input value={firstName} onChange={(event) => setFirstName(event.target.value)} required maxLength={100} /></label><label className="field"><span>Фамилия</span><input value={lastName} onChange={(event) => setLastName(event.target.value)} maxLength={100} /></label><label className="field"><span>Дата рождения</span><input type="date" value={birthDate} onChange={(event) => setBirthDate(event.target.value)} /></label><button className="secondary-button" disabled={!firstName.trim() || update.isPending}>{update.isPending ? "Сохраняем…" : "Сохранить изменения"}</button></form> : <p className="muted">{crmManaged ? "Эти поля управляются CRM и станут локально редактируемыми после отключения интеграции." : "У вас есть доступ только к просмотру."}</p>} {editable && !crmManaged && <><hr /><div className="danger-zone"><h3>{item.status === "ACTIVE" ? "Архив" : "Восстановление"}</h3><p className="muted">{item.status === "ACTIVE" ? "Архивный ребёнок не появляется в новых занятиях." : "Верните ребёнка в активный список."}</p><button className={item.status === "ACTIVE" ? "danger-button" : "secondary-button"} disabled={archive.isPending} onClick={() => archive.mutate()}>{item.status === "ACTIVE" ? "Архивировать" : "Восстановить"}</button></div></>}</section></div></>;
}
