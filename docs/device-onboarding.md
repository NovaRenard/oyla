...
1. Центр и первого OWNER создаёт оператор через `oyla-admin create-center`; публичной страницы регистрации нет.
2. Владелец входит на `/login`.
...
8. В карточке `/devices/:deviceId` можно изменить имя и роль, заблокировать/разблокировать или отвязать устройство. Отвязка всегда требует подтверждения. ACTIVE и BLOCKED устройства нельзя подключить кодом другого центра; только после unlink та же установка может быть подключена снова и получит новый token.
...
2. Создайте два центра через `oyla-admin create-center` и войдите в первый.
